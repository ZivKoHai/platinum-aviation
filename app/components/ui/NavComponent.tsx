    ...
    <header className="bg-background sticky top-0 z-50">
      ...
      >
        ...
        <div className="hidden lg:flex lg:flex-1 lg:justify-end">
          <Button href={globalConfig.whatsappLink}>Contact Us</Button>
        </div>
      </nav>
      <Dialog
        as="div"
              ...
              <div className="space-y-2 py-6">
                {Links.map((item) => (
                  <Link
                    ...
                    href={item.href}
                    onClick={() => setMobileMenuOpen(false)}
                    className="-mx-3 block rounded-lg px-3 py-2 text-base font-semibold leading-7 text-foreground hover:bg-gray-400"
                  >
                    {item.name}
                  </Link>
                ))}
